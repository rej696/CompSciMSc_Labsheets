...
\begin{itemize}
	...
	\item[] Number States
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				% states
				\node[state, initial] (p) {1};
				\node[state, right of=p] (s) {2};
				\node[state, right of=s] (q) {3};
				\node[state, below of=s, accepting] (r) {4};
	...
	\item[] Consolidate transition from 1 to 4
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				% states
				...
				(q) edge[bend left, below] node{x} (s)
				(p) edge[above] node{$\varepsilon$} (s)
				(p) edge[below] node{$\varepsilon$} (r)
				(s) edge[right] node{$\varepsilon$} (r);
			\end{tikzpicture}
		\end{figure}
	
	\item[] Consolidate transition from 3 to 4
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				% states
	...
	\item[] Concatenate transition from 1->2->3 (i, j, k)
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				% states
				...
				% transitions
				\draw (s) edge[above] node{x} (q)
				(q) edge[bend left, below] node{x} (s)
				(p) edge[bend left, above] node{x} (q)
				(q) edge[below] node{x} (r)
				(p) edge[above] node{$\varepsilon$} (s)
				(p) edge[below] node{$\varepsilon$} (r)
				(s) edge[right] node{$\varepsilon$} (r);
	...
	\item[] Concatenate transitions from 2->3->2 (i, j, k)
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				% states
				...
				(q) edge[bend left, below] node{x} (s)
				(q) edge[below] node{x} (r)
				(q) edge[loop right] node{$x \circ x$} (q)
				(p) edge[above] node{$\varepsilon$} (s)
				(p) edge[below] node{$\varepsilon$} (r)
				(s) edge[right] node{$\varepsilon$} (r);
			\end{tikzpicture}
	...
	\item[] All transitions that go through state 2 have been consolidated, so we
		can remove it
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				% states
				\node[state, initial] (p) {1};
				\node[state, right of=p] (q) {3};
				\node[state, below of=p, accepting] (r) {4};

				% transitions
				\draw (q) edge[right] node{x} (r)
				(p) edge[above] node{x} (q)
				(q) edge[loop right] node{$x \circ x$} (q)
				(p) edge[left] node{$\varepsilon$} (r);
			\end{tikzpicture}
		\end{figure}

	\item[] Concatenate/Star Transitions from 1->3->4 (i,j,k)
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}[
					node distance=3cm
				...
				% states
				\node[state, initial] (p) {1};
				\node[state, above of=p] (q) {3};
				\node[state, right of=p, accepting] (r) {4};

				% transitions
				\draw (q) edge[right] node{x} (r)
				(p) edge[left] node{x} (q)
				(q) edge[loop left] node{$x \circ x$} (q)
				(p) edge[above] node{$\varepsilon$} (r)
				(p) edge[bend right, below] node{$x \circ (x \circ x)^\ast \circ x$} (r);
			\end{tikzpicture}
		\end{figure}

	\item[] All transitions that go through state 3 have been consolidated, so we
		can remove it
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}[
				...
				% transitions
				\draw (p) edge[bend left, above] node{$\varepsilon$} (r)
				(p) edge[bend right, below] node{$x \circ (x \circ x)^\ast \circ x$} (r);
			\end{tikzpicture}
		\end{figure}

	\item[] Union the remaining transitions
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}[
					node distance=5cm
				...
				% transitions
				\draw (p) edge[above] node{$\varepsilon \cup (x (x x)^\ast x)$} (r);
			\end{tikzpicture}
		\end{figure}
...
The regular expression derived above is not the simplest expression of the
language. This would be $(xx)^\ast$. However, it is easily seen that these two
regular expressions are equivalent.

\newpage
\subsubsection{NFA to RE activity}
My NFA is the same as before:
	...
	\item[] Initial NFA:
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				% states
				\node[state, initial] (s) {};
				\node[state, accepting, right of=s] (p) {};
				\node[state, right of=p] (q) {};

				% transitions
				\draw (s) edge[above] node{x} (p)
				(p) edge[bend left, above] node{x} (q)
				(q) edge[bend left, below] node{x} (p);
			\end{tikzpicture}
		\end{figure}
	...
	\item[] Ensure no outgoing transitions from accept state
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				% states
				\node[state, initial] (s) {};
				\node[state, right of=s] (p) {};
				\node[state, right of=p] (q) {};
				\node[state, below of=p, accepting] (r) {};

				% transitions
				\draw (s) edge[above] node{x} (p)
				(p) edge[bend left, above] node{x} (q)
				(q) edge[bend left, below] node{x} (p)
				(p) edge[right] node{$\varepsilon$} (r);
	...
	\item[] Concatenate transition from 1->2->3 (i, j, k)
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				% transitions
				\draw (s) edge[above] node{x} (p)
				(s) edge[bend left, above] node{xx} (q)
				(p) edge[above] node{x} (q)
				(q) edge[bend left, below] node{x} (p)
				(p) edge[right] node{$\varepsilon$} (r)
				(s) edge[left] node{x} (r)
	...
	\item[] Concatenate/Star Transitions from 1->3->4 (i,j,k)
		\begin{figure}[H]
			...
			\begin{tikzpicture}[
				...
				% states
				\node[state, initial] (s) {1};
				\node[state, above of=s] (q) {3};
				\node[state, right of=s, accepting] (r) {4};

				% transitions
				\draw (s) edge[left] node{xx} (q)
				(q) edge[loop left] node{$x \circ x$} (q)
				(s) edge[above] node{x} (r)
				(s) edge[bend right, below] node{$x \circ x \circ (x \circ x)^\ast \circ x$} (r)
				(q) edge[right] node{x} (r);
	...
	\item[] All transitions that go through state 3 have been consolidated, so we
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}[
				...
				% transitions
				\draw (s) edge[above] node{x} (r)
				(s) edge[bend right, below] node{$x \circ x \circ (x \circ x)^\ast \circ
				x$} (r);
			\end{tikzpicture}
		\end{figure}
	...
	\item[] Union the remaining transitions
		\begin{figure}[H]
			...
			\begin{tikzpicture}[
				...
				% transitions
				\draw (s) edge[above] node{$(xx (x \circ x)^\ast
				x) \cup x$} (r);
			\end{tikzpicture}
		\end{figure}
\end{itemize}

The regular expression $(xx (x \circ x)^\ast x) \cup x$ is equivalent to
$x(xx)^\ast$. Both these regular expressions represent the language with an odd
number of x's.
